...
{\small\color{graytext}技术助理\hspace{0.3em}|\hspace{0.3em}昆明\hspace{0.3em}|\hspace{0.3em}工程物探与检测}

\vspace{2pt}

\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    \item 协助完成伺服电机实验准备与测试试验机调试，参与传感器信号采集、线路参数检查与设备运行状态监测，掌握嵌入式设备调试与数据采集的基本流程。
    \item 负责实验数据录入、分类汇总与复核，使用 Git 进行技术文档版本管理；配合项目现场检测工作，了解工程检测流程与传感器设备使用规范。
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    ...
    \item \textbf{上位机 + 数据层}：独立实现 C\# 三层架构桌面端（MQTT 实时订阅 + Excel 导入双通道），DAL 参数化查询防注入，BLL 主键查重，形成"采集 → Broker → 上位机 → MySQL"端到端数据流。
\end{itemize}

\vspace{0.15em}

% --- 智能交互台灯 ---
\projectblock{STM32 智能交互台灯控制系统}{2025.01 -- 2026.05}{技术栈：STM32F103C8T6\hspace{0.3em}|\hspace{0.3em}光敏ADC\hspace{0.3em}|\hspace{0.3em}HC-SR04\hspace{0.3em}|\hspace{0.3em}SNR8016语音\hspace{0.3em}|\hspace{0.3em}HC-05蓝牙\hspace{0.3em}|\hspace{0.3em}PWM\hspace{0.3em}|\hspace{0.3em}OLED}
