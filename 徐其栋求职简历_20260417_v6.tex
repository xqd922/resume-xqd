...
% ======================== 头部 ========================
\begin{center}
    {\fontsize{22}{26}\selectfont\bfseries\color{primary} 徐其栋}
...
\textbf{通信与联网} & I2C、UART、MQTT、蓝牙、Wi-Fi \\[3pt]
\textbf{数据库} & MySQL、ADO.NET、SQL 参数化查询 \\[3pt]
\textbf{开发工具} & Keil MDK、Visual Studio、Git、Linux、嘉立创EDA
\end{tabular}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    ...
    \item 搭建多通信链路：I2C 统一驱动 PCA9685 与 MPU6050；USART2 蓝牙网页遥控、UART4 ESP32 语音识别，落地双通道交互；WS2812 反馈系统状态。
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    ...
    \item \textbf{数据持久化}：DAL 封装 MySQLHelper 统一参数化查询防 SQL 注入，BLL 主键查重与行级异常隔离，app.config 外部化配置；形成"设备 $\to$ Broker $\to$ 上位机 $\to$ MySQL"端到端链路。
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    ...
    \item 打样回板后完成手工焊接、上电自检与故障排查（万用表 / 示波器验证电压与时钟波形），形成"原理图 $\to$ PCB $\to$ 实物 $\to$ 调试"硬件开发闭环。
\end{itemize}
...
% ======================== 底部标签 ========================
\begin{center}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {STM32};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {ESP32};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {I2C / UART};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {MQTT};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {C\# / .NET};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {MySQL};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {PCB设计};}
\hspace{0.2em}
\tikz{\node[fill=tagbg, rounded corners=3pt, inner sep=4pt, text=tagtext, font=\small\bfseries] {Git / Linux};}
\end{center}
