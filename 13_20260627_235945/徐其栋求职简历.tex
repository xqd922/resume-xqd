...
{\color{darktext}
\textbf{主修课程：}C 语言、Python、单片机原理、嵌入式技术、计算机网络、传感器原理、数字电子技术
}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, parsep=0pt, topsep=2pt]
    \item ESP32 基于 MQTT 上报传感器数据至 EMQX Broker；独立实现 C\# 三层架构桌面端，支持 MQTT 实时订阅与 Excel 批量导入，DAL 参数化查询防注入，端到端覆盖物联网全栈。
\end{itemize}
...
2026 届物联网工程本科毕业生，具备 STM32、ESP32 项目实战经验，熟悉 I2C、UART、MQTT、蓝牙等通信方式，能独立完成硬件连接、驱动调试与功能验证；掌握 C\# + MySQL 三层架构上位机开发与 Git 版本管理，可快速进入嵌入式开发与测试工作。
