...
\textbf{编程语言} & C语言、Python \\[6pt]
\textbf{嵌入式开发} & STM32、ESP32、外设驱动调试 \\[6pt]
\textbf{通信与联网} & I2C、UART、MQTT、蓝牙通信 \\[6pt]
\textbf{开发工具} & Keil MDK、Git、Linux、嘉立创EDA \\[6pt]
...
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, parsep=0pt, topsep=4pt]
    \item 以 STM32F407ZE（168MHz）为主控，结合 PCA9685 的 16 路 12 位 PWM 驱动 8 路舵机（四腿 $\times$ 2 关节），实现前进、后退、左右转、站立、蹲下、鞠躬、挥手等 9 类基础动作。
    \item 基于余弦定理与三角函数完成腿部运动学逆解：由足尖目标坐标 $(x, y)$ 反推大小腿舵机角度，并通过步进插值算法实现舵机平滑过渡，避免机械冲击与抖动。
    \item 搭建多通信链路：I2C 统一驱动 PCA9685 与 MPU6050 姿态传感器；USART2 对接蓝牙模块实现网页遥控；UART4 对接 ESP32 完成语音识别控制，落地蓝牙、语音两种交互方式。
    \item 精确时序驱动 WS2812 地址可控 LED，通过 GRB 颜色反馈运行 / 故障等系统状态；独立完成硬件接线、驱动编写、整机联调与异常排查，并输出课程设计报告。
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, itemsep=4pt, parsep=0pt, topsep=4pt]
    \item 学习并理解 MQTT 发布/订阅通信模型，掌握 Topic、QoS、Keep Alive、遗嘱消息等核心概念。
    \item 结合 ESP32 物联网开发场景，理解设备数据上报、状态订阅与远程控制的典型通信流程。
    \item 能基于 MQTT 协议完成设备与服务端之间的基础消息交互设计，并用于物联网系统通信方案分析。
    \item 通过该部分学习强化了我对轻量级物联网协议选型与通信链路设计的理解。
\end{itemize}
...
2026届物联网工程专业本科生，求职方向为嵌入式开发/物联网工程师。具备 STM32、ESP32 项目实践经验，能够独立完成基础嵌入式项目的硬件连接、驱动调试、通信联调与功能验证。熟悉 I2C、UART、MQTT、蓝牙等常用通信方式，具备 PCB 设计、焊接调试与基础前端展示能力，能够较快进入实际开发与测试工作。
